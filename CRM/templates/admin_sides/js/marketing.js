...
document.addEventListener('DOMContentLoaded', () => {
    const config = { endpoint: '/api/sales_marketing', amountKey: 'project_value', projectKey: 'project_name', metricKey: 'status', metricType: 'won', entryLabel: 'Campaign records', amountLabel: 'Pipeline value', metricSubLabel: 'Won opportunities', icons: ['bi-megaphone', 'bi-cash-stack', 'bi-trophy', 'bi-briefcase'] };
    ...
    const refreshMarketingSummary = () => fetch(config.endpoint).then((response) => response.ok ? response.json() : null).then((result) => {
        ...
        cards.querySelector('[data-summary="metric"]').textContent = rows.filter((row) => String(row[config.metricKey] || '').toLowerCase() === 'won').length;
        cards.querySelector('[data-summary="projects"]').textContent = projects.size;
    }).catch(() => {});
    refreshMarketingSummary();
    document.addEventListener('marketing-data-updated', refreshMarketingSummary);
});
...
function setupTableChart(config) {
    ...
    button.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            const chartRows = getRows();
            if (summary) {
                const pipelineValue = chartRows.reduce((total, row) => total + (Number.parseFloat(String(row[valueKey] ?? '').replace(/[^\d.-]/g, '')) || 0), 0);
                const won = chartRows.filter((row) => String(row.status || '').toLowerCase() === 'won').length;
                summary.innerHTML = [['Opportunities', chartRows.length], ['Pipeline Value', pipelineValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })], ['Won', won]].map(([label, value]) => `<div class="dashboard-summary-item"><span class="dashboard-summary-value">${value}</span><span class="dashboard-summary-label">${label}</span></div>`).join('');
            }
            const labels = [];
            const data = [];
...
async function loadMarketingRows() {
    ...
    try {
        ...
        const rows = Array.isArray(result.data) ? result.data : [];
        marketingRows = rows;
        document.dispatchEvent(new Event('marketing-data-updated'));
...
setupTableChart({ buttonId: 'chartToggleBtn', modalId: 'marketingChartModal', canvasId: 'marketingChart', getRows: () => marketingRows, labelKey: 'source', valueKey: 'project_value', summaryId: 'marketingChartSummary' });
